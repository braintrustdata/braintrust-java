...
class ExperimentListPageAsync private constructor(private val experimentService: ExperimentServiceAsync, private val params: ExperimentListParams, private val response: Response, ) {
    ...
    fun getNextPage(): CompletableFuture<Optional<ExperimentListPageAsync>> {
      return getNextPageParams().map {
        experimentService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: ExperimentListPageAsync, ) {

        fun forEach(action: Predicate<Experiment>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<ExperimentListPageAsync>>.forEach(action: (Experiment) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {

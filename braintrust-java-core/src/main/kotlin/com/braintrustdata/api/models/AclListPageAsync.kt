...
class AclListPageAsync private constructor(private val aclsService: AclServiceAsync, private val params: AclListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<AclListParams> {
      ...
      return if (params.endingBefore().isPresent) {
        Optional.of(AclListParams.builder().from(params).endingBefore(objects().first().id()).build());
      } else {
        Optional.of(AclListParams.builder().from(params).startingAfter(objects().last().id()).build());
      }
    }

    fun getNextPage(): CompletableFuture<Optional<AclListPageAsync>> {
      return getNextPageParams().map {
        aclsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: AclListPageAsync, ) {

        fun forEach(action: Predicate<Acl>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<AclListPageAsync>>.forEach(action: (Acl) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {

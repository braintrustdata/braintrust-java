...
class GroupListPageAsync private constructor(private val groupService: GroupServiceAsync, private val params: GroupListParams, private val response: Response, ) {
    ...
    fun getNextPage(): CompletableFuture<Optional<GroupListPageAsync>> {
      return getNextPageParams().map {
        groupService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: GroupListPageAsync, ) {

        fun forEach(action: Predicate<Group>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<GroupListPageAsync>>.forEach(action: (Group) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {

...
class ProjectTagListPageAsync private constructor(private val projectTagService: ProjectTagServiceAsync, private val params: ProjectTagListParams, private val response: Response, ) {
    ...
    class AutoPager constructor(private val firstPage: ProjectTagListPageAsync, ) {
        ...
        fun forEach(action: Predicate<ProjectTag>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<ProjectTagListPageAsync>>.forEach(action: (ProjectTag) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {

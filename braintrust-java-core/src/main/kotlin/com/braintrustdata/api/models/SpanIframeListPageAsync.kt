...
class SpanIframeListPageAsync private constructor(
    ...
    fun getNextPageParams(): Optional<SpanIframeListParams> {
      ...
      return if (params.endingBefore().isPresent) {
        Optional.of(SpanIframeListParams.builder().from(params).endingBefore(objects().first().id()).build());
      } else {
        Optional.of(SpanIframeListParams.builder().from(params).startingAfter(objects().last().id()).build());
      }
    }

    fun getNextPage(): CompletableFuture<Optional<SpanIframeListPageAsync>> {
      return getNextPageParams().map {
        spanIframesService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<SpanIFrame>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<SpanIframeListPageAsync>>.forEach(action: (SpanIFrame) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {

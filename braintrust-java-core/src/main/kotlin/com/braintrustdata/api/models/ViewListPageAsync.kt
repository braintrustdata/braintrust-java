...
class ViewListPageAsync private constructor(private val viewService: ViewServiceAsync, private val params: ViewListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<ViewListParams> {
      ...
      return if (params.endingBefore().isPresent) {
        Optional.of(ViewListParams.builder().from(params).endingBefore(objects().first().id()).build());
      } else {
        Optional.of(ViewListParams.builder().from(params).startingAfter(objects().last().id()).build());
      }
    }
    ...
    class AutoPager constructor(private val firstPage: ViewListPageAsync, ) {

        fun forEach(action: Predicate<View>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<ViewListPageAsync>>.forEach(action: (View) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {

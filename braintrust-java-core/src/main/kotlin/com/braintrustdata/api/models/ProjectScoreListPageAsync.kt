...
class ProjectScoreListPageAsync private constructor(private val projectScoreService: ProjectScoreServiceAsync, private val params: ProjectScoreListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<ProjectScoreListParams> {
      ...
      return if (params.endingBefore().isPresent) {
        Optional.of(ProjectScoreListParams.builder().from(params).endingBefore(objects().first().id()).build());
      } else {
        Optional.of(ProjectScoreListParams.builder().from(params).startingAfter(objects().last().id()).build());
      }
    }
    ...
    class AutoPager constructor(private val firstPage: ProjectScoreListPageAsync, ) {

        fun forEach(action: Predicate<ProjectScore>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<ProjectScoreListPageAsync>>.forEach(action: (ProjectScore) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
